...
for algorithm in ['SAMME', 'SAMME.R']:
    for n_estimators in [10,50,100,200,500,1000,1500,2000]:
        try:
            print ("Excec AdaBoost with {} - {}".format(algorithm, n_estimators))
...
for bootstrap in [True, False]:
    for n_estimators in [10,50,100,200,500,1000,1500,2000]:
        try:
            print ("Excec Bagging with {} - {}".format(bootstrap, n_estimators))
...
for criterion in ['gini', 'entropy']:
    for splitter in ['best', 'random']:
        try:
            print ("Excec DecisionTree with {} - {}".format(criterion, splitter))
...
for loss in ['deviance', 'exponential']:
    for n_estimators in [10,50,100,200,500,1000,1500,2000]:
        try:
            print ("Excec GradientBoostingClassifier with {} - {} - {} - {}".format(loss, n_estimators, 2, 1))
            gradientObject = Gradient.Gradient(data,target,n_estimators, loss, 2, 1, 10)
    ...
    for algorithm in ['auto', 'ball_tree', 'kd_tree', 'brute']:
        for metric in ['minkowski', 'euclidean']:
            for weights in ['uniform', 'distance']:
                try:
                    print ("Excec KNeighborsClassifier with {} - {} - {} - {}".format(n_neighbors, algorithm, metric, weights))
