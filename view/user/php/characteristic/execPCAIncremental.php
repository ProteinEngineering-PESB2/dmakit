<?php
  ...
  $query = "insert into job values ($idJob, '$nameJob', '$descJob', NOW(), NOW(), $idUSer, '$nameDocument', 'START', 'PCA-INCREMENTAL', '-')";
  $query2 = "insert into dataSet values ($idJob, '$nameDocument', NOW(), NOW(), $idUSer, 'PCA-INCREMENTAL', $idJob)";
  ...
  if ($requestData == "BIEN"){#movemos el archivo de tmp al path del usuario y ejecutamos el proceso solo si la opcion de algorithm es todos...
    ...
    $command = "python3 /var/www/html/dmakitWeb/model/launcherWebFeatureAnalysis.py $idUSer $idJob $pathMove$nameDocument $pathRespone 6 $optionScale 0";
    exec($command);
