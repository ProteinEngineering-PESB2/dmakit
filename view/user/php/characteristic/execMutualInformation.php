<?php
  ...
  $query = "insert into job values ($idJob, '$nameJob', '$descJob', NOW(), NOW(), $idUSer, '$nameDocument', 'START', 'MUTUAL-INFORMATION', '-')";
  $query2 = "insert into dataSet values ($idJob, '$nameDocument', NOW(), NOW(), $idUSer, 'MUTUAL-INFORMATION', $idJob)";
  ...
  if ($requestData == "BIEN"){#movemos el archivo de tmp al path del usuario y ejecutamos el proceso solo si la opcion de algorithm es todos...
    ...
    $command = "python3 /var/www/html/dmakitWeb/model/launcherWebFeatureAnalysis.py $idUSer $idJob $pathMove$nameDocument $pathRespone 4 $optionScale 0";
    exec($command);
