<?php
  ...
  $query = "insert into job values ($idJob, '$nameJob', '$descJob', NOW(), NOW(), $idUSer, '$nameDocument', 'START', 'STATISTICAL-ANALYSIS', '-')";
  $query2 = "insert into dataSet values ($idJob, '$nameDocument', NOW(), NOW(), $idUSer, 'STATISTICAL-ANALYSIS', $idJob)";
  ...
  if ($requestData == "BIEN"){#movemos el archivo de tmp al path del usuario y ejecutamos el proceso solo si la opcion de algorithm es todos...
    ...
    $pathActual = "/var/www/html/dmakitWeb/dataStorage/tmp/statisticalTest/$nameDocument";
    ...
    $command = "python3 /var/www/html/dmakitWeb/model/LauncherStatisticalTest.py $pathMove$nameDocument $option";
    $output = [];
    exec($command, $output);
